...
- 제한시간은 **30분** — 넘기면 자동으로 게임이 종료됩니다.
- 12레벨을 모두 클리어하면 **클리어**로 기록되고, 남은 시간 1초당 1점이 점수에 더해집니다(실수해도 감점이 없어 클리어 기록끼리 점수가 다 같아지는 것을 막고, 빨리 깰수록 점수가 높아지게 함). 클리어했든 안 했든 **점수**로 순위가 매겨지며, 클리어 기록이 항상 미클리어 기록보다 순위가 높습니다.
